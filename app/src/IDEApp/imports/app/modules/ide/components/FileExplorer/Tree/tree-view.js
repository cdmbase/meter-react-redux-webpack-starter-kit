...
import * as JobsActions from '../../../../application/action-types';
import { file } from '../../../actions/boxes-actions';
import Loader from 'react-loader';
import { JOB_STATUS_IN_PROGRESS } from '../../../../application/action-types';
        ...
        return (
            <Loader loaded={job && job.status != JOB_STATUS_IN_PROGRESS}>
                <div className="project-workspace workspace tree-view">
                    ...
                    {task ? (
                        [JOB_ACTION_CREATION, JOB_ACTION_RENAME].includes(task.data.type) ? (
                            console.log(task)
                        ) : null) : null}
                    {task ? (
