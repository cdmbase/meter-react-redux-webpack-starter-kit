...
export default class ContextMenu extends Component {
    ...
    click(event, module) {
        const { act } = module;
        console.log("[LOG] [ContextMenu][Click]", { module, act });
        switch(act) {
            case ACT_OPEN:
    ...
    modal(type, options) {
        console.log("[LOG][ContextMenu]", { type, options });
    ...
    render() {
        let { item = {} } = this.props;
        let { type } = item;
        console.log("[LOG][ContextMenu][Render]", { props: this.props, item: item.module });
        return (
            !!item &&
